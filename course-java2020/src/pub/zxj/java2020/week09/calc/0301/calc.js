    ...
    for (let objButtonKey in objButton) {
        objButton[objButtonKey].onclick = function () {
            ...
            if (isNumber(this.value)) {
                ...
                switch (this.value) {
                    case "+":
                        btn_values = parseInt(monitor.value);
                        monitor.value = 0;
                        operator = "+";
                        break;
                    case "-":
                    ...
                    case "=":
                        switch (operator) {
                            case "+":
                                monitor.value = btn_values + parseInt(monitor.value);
                            ...
                            case "*":
                                ...
                                break;
                            case "/":
                                monitor.value = (parseInt(monitor.value) === 0) ? 0 : btn_values / parseInt(monitor.value);
                                // console.log(btn_values);
                                break;
                            case "%":
                                monitor.value = btn_values % parseInt(monitor.value);
