package com.imooc.course1183.chapter07;
 ...
 */
public class Sections0701Practice {
    public static void main(String[] args) {
        ...
        //提示信息
        System.out.print("您要开始第几周学习啦, 直接输入数字吧(1 - 35):");

        //设置变量存储接收到的数据
        int week = new Scanner(System.in).nextInt();
        int weekNum = 4;
        if (week > (contentList.length + 1)) {
            System.out.println("您输入的数字有误！");
        } else {

            //计算今天是几月(1-月第一周、4-月第4周)
            int month = ((week % 4 == 0) ? week / 4 : week / 4 + 1);
            ...
            for (int i = (month - 1) * weekNum; i < month * weekNum && i < contentList.length; i++) {
                if ((i + 1) == week) {
                    System.out.print("√");
                }
